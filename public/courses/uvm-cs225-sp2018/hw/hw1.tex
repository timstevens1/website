...
Recall the set of boolean arithmetic terms $\mathcal{T}$ and metafunctions size and depth:
\[ \begin{array}{l}
   t \in \mathcal{T} \Coloneqq T \mid F \mid \texttt{if}\ t\ \texttt{then}\ t\ \texttt{else}\ t
   \\
   \\ \text{size}(T) \coloneqq 1
   \\ \text{size}(F) \coloneqq 1
   \\ \text{size}(\texttt{if}\ t_1\ \texttt{then}\ t_2\ \texttt{else}\ t_3) \coloneqq \text{size}(t_1) + \text{size}(t_2) + \text{size}(t_3)
   \\
   \\ \text{depth}(T) \coloneqq 1
   \\ \text{depth}(F) \coloneqq 1
   \\ \text{depth}(\texttt{if}\ t_1\ \texttt{then}\ t_2\ \texttt{else}\ t_3) \coloneqq \text{max}(\text{depth}(t_1),\text{depth}(t_2),\text{depth}(t_3))
   \end{array}
\]
Define some term $t \in \mathcal{T}$ such that $\text{size}(t) = 7$ and $\text{depth}(t) = 2$.

\subsubsection*{Problem 3 (25 points)}

Either prove by structural induction that $\text{size}(t)$ always produces an
odd number, or give a counter-example which shows $\text{size}(t)$ can produce
an even number.
