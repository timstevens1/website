...
*-+-rdistrib (suc m) n p rewrite *-+-rdistrib m n p | sym (+-assoc p (m * p) (n * p)) = refl
...
module *-comm-lemmas where
  ...
  *-runit : ∀ (m : ℕ) → m * 1 ≡ m
  *-runit zero = refl
  *-runit (suc m) rewrite *-runit m = refl

  -- (Hint: proceed by induction on m.)
  -- (Hint: use rewrite.)
  -- (Hint: use lemmas.)
  *-+-ldistrib : ∀ (m n p : ℕ) → m * (n + p) ≡ m * n + m * p
  *-+-ldistrib zero n p = refl
  *-+-ldistrib (suc m) n p
...
-- (Hint: use lemmas.)
*-comm : ∀ (m n : ℕ) → m * n ≡ n * m
*-comm zero n rewrite *-rzero n = refl
*-comm (suc m) n rewrite *-+-ldistrib n 1 m | *-runit n | *-comm m n = refl
