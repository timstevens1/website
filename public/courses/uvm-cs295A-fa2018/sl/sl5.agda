...
-- HINT: do induction on `m` and `n`
cor[≡?]-L : ∀ (m n : ℕ) → m ≡ n → (m ≡? n ≡ true)
cor[≡?]-L zero zero refl = refl
cor[≡?]-L zero (suc n) ()
cor[≡?]-L (suc m) zero ()
cor[≡?]-L (suc m) (suc .m) refl = cor[≡?]-L m m refl
...
-- HINT: do induction on `m` and `n`
cor[≡?]-R : ∀ (m n : ℕ) → (m ≡? n ≡ true) → m ≡ n
cor[≡?]-R zero zero ε = refl
cor[≡?]-R zero (suc n) ()
cor[≡?]-R (suc m) zero ()
cor[≡?]-R (suc m) (suc n) ε rewrite cor[≡?]-R m n ε = refl
...
reverse-⧺-dist [] ys rewrite ⧺-runit (reverse ys) = refl
reverse-⧺-dist (x ∷ xs) ys rewrite reverse-⧺-dist xs ys | ⧺-assoc (reverse ys) (reverse xs) [ x ] = refl
...
reverse-involutive (x ∷ xs) rewrite reverse-⧺-dist (reverse xs) [ x ] | reverse-involutive xs = refl
...
map-∘-dist g f [] = refl
map-∘-dist g f (x ∷ xs) rewrite map-∘-dist g f xs = refl
