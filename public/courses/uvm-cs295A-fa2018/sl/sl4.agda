...
leading-1-two = [x0] (bits x1) [one]

leading-1-three : Leading-1 three
leading-1-three = [x1] (bits x1) [one]
...
not-leading-1-one ([x1] .(bits x0) ([x0] .bits ()))
...
leading-one-inc .(bits x1) [one] = [x0] (bits x1) [one]
leading-one-inc .(n x0) ([x0] n l) = [x1] n l
leading-one-inc .(n x1) ([x1] n l) = [x0] (inc n) (leading-one-inc n l)
...
bin-+-lsuc bits bits = refl
bin-+-lsuc bits (n x0) = refl
bin-+-lsuc bits (n x1) = refl
bin-+-lsuc (m x0) bits = refl
bin-+-lsuc (m x0) (n x0) = refl
bin-+-lsuc (m x0) (n x1) = refl
bin-+-lsuc (m x1) bits = refl
bin-+-lsuc (m x1) (n x0) rewrite bin-+-lsuc m n = refl
bin-+-lsuc (m x1) (n x1) rewrite bin-+-lsuc m n = refl
...
bin-+-*2 .(bits x1) [one] = refl
bin-+-*2 .(n x0) ([x0] n l) rewrite bin-+-*2 n l = refl
bin-+-*2 .(n x1) ([x1] n l) rewrite bin-+-*2 n l = refl
...
to∘+ zero n = refl
to∘+ (suc m) n rewrite to∘+ m n | bin-+-lsuc (to m) (to n) = refl
...
to∘from-zero n e rewrite e = refl
...
to∘from-leading-1 .(n x1) ([x1] n l) rewrite to∘+ (from n) (from n) | to∘from-leading-1 n l | bin-+-*2 n l = refl
...
to∘from n (inj₁ e) = to∘from-zero n e
to∘from n (inj₂ l) = to∘from-leading-1 n l
